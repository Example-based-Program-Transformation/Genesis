...
import genesis.repair.validation.FixBenchNoTestingOracle;
...
//		ValidationOracle oracle = new TestingOracle(manager);
		ValidationOracle oracle = new FixBenchNoTestingOracle(manager);
		...
		for (SuspiciousLocation loc : locs) {
			System.out.println("Checking one suspicious location");
			logTimingStatistics();
			...
			System.out.println(" fetchResult len = " + fetchResult.size());
			for (Map.Entry<MyCtNode, HashSet<MyNodeSig>> e : fetchResult.entrySet()) {
				System.out.println("\tChecking one fetchResult");
				MyCtNode root;
				...
				System.out.println(" candidatePairs len = " + candidatePairs.size());
				for (Pair<HashSet<MyNodeSig>, MyCtNode> p : candidatePairs) {
					//System.out.println(p.y);
					System.out.println("\tChecking one candidatePair");
					...
					System.out.println(" candidatePatches len = " + candidatePatches.size());
