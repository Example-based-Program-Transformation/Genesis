...
3. Run FixBenchDefectLocationProducer to create localization.log file
...
3. Use FixBenchDefectLocationProducer.java
mvn exec:java -Dexec.mainClass="genesis.infrastructure.FixBenchDefectLocationProducer" -Dexec.args="/home/root/Workspace/bowen_bench/PTBench-Data-master/NULL_DEREFERENCE/150/pair.info /home/root/Workspace/bowen_bench/PTBench-Data-master/NULL_DEREFERENCE/150/diff.txt"


run like this:
python python/fixbench_create_case_for_rq2.py null_deference_174 /home/root/Workspace/PTBench-Data/NULL_DEREFERENCE/174  custom_case_null_deference_174 custom-space
...
def download_repo_as_case_directory(path_to_example, case_dir_name, id):
    ...
    def download(github_repo, id):
        ...
        sys.stderr.write('starting git checkout ...  for fixed commit  of ... ' + 'custom_case_' + id + ' : ' + before_commit + '\n')
    ...
    new_commit = get_new_commit(info)

    write_test_case_file()

    write_case_conf_file()

    repo_info = git_repo(info, new_commit)
...
if __name__ == '__main__':
    ...
    print('fixbench_create_case_for_rq2.py:')
    print(sys.argv)
    id = sys.argv[1]
    ...
    move_generated_patches(id + "_" + trained_space.split('/')[-1])
